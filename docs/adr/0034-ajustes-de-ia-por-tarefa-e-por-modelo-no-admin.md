...
3. **Defaults por tarefa** (`TASK_DEFAULT_SETTINGS`): Geração `effort: medium`, thinking no default do modelo (teto de 60s da rota). `effort` só vai para Opus, Sonnet e Fable: uma linha legada em outro modelo (ex.: Haiku, que dá 400 com `effort`) roda no default dele. Tradução e Extração `thinking: off`, effort no default (tarefas fiéis; com thinking, os tokens dele dividem o teto com o JSON). Os defaults são seguros para o modelo default de cada tarefa. Com thinking que pode ligar, Tradução e Extração ganham 8k tokens de folga no `max_tokens`.
...
5. **O que a API não informa é testado ao salvar.** A Models API não diz se o thinking pode ser DESLIGADO (Opus 5.5 e Fable recusam `disabled` com 400). Ao salvar, o servidor faz uma chamada mínima (`max_tokens` 512, 20s, sem retry) com o modelo e o ajuste. 400/404 ⇒ `ajuste_recusado` com a mensagem da Anthropic e nada é gravado. Falha de rede/credencial/limite/5xx, ou 400 de conta sem crédito ⇒ grava sem verificar (queda da API não trava o admin). O `defaultModel` legado do PUT passa pelos mesmos portões, com o ajuste salvo para o modelo. As chamadas de teste rodam antes da escrita; o jsonb é regravado numa transação com lock da linha, para dois salvamentos concorrentes não se sobrescreverem. As rotas que chamam a Anthropic com esses ajustes (admin, Extração, tradução sob demanda) têm `maxDuration = 60`, e a Extração usa o mesmo prazo de 50s da Geração. Rejeitado: tabela de "quem desliga o thinking" em código, que envelhece a cada lançamento.
