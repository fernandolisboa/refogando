...
> **Nota (#470, 2026-09-25):** a **verificação de e-mail** saiu do diferido (D3) para fechar a **enumeração de contas no cadastro** (`/sign-up/email` respondia 422 `USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL` só quando o e-mail já tinha conta). `requireEmailVerification: true` liga a resposta genérica do Better Auth: e-mail existente responde **200 igual a um novo**, sem cookie, sem criar/alterar nada e sem e-mail. O corpo do cadastro é reduzido a `{ token: null, user: { id, name, email, emailVerified, image, createdAt, updatedAt } }` (hook `after`) e o id sintético é uuid (`customSyntheticUser`) — sem isso o usuário sintético se denunciava pela forma (sem `handle`, `role`/`plan` null, id base62). O cadastro **não loga mais**: o link (JWT de 24h, pelo mesmo `sendAccountEmail`/Brevo do reset, idioma `users.locale` → cookie `locale` → Accept-Language) confirma e **já loga** (`autoSignInAfterVerification`), voltando pela tela `/{locale}/verify-email` (link inválido ⇒ reenvio). Login de conta não confirmada: o Better Auth responde 403 `EMAIL_NOT_VERIFIED` só **depois** de conferir a senha — oráculo (quem cadastra o e-mail alheio com a senha P e depois entra com P distingue "já existia" de "conta nova"). O hook `after` **reescreve** esse 403 no **mesmo 401 `INVALID_EMAIL_OR_PASSWORD`** da senha errada (resposta pronta, porque um `APIError` lançado no `after` mantém o status original); o reenvio automático (`sendOnSignIn`, só com a senha certa) continua, com o `returnTo` levado no header `x-refogando-return-to` (sem `callbackURL` no corpo, que faria o cliente redirecionar a página). A tela de Entrar, com a confirmação ligada, mostra junto do erro uma dica neutra ("Acabou de criar a conta? Confirme seu email…") + reenvio. Reenvio público neutro (3/min por IP) e **no máx. 3 e-mails de confirmação por conta a cada 15 min** (contagem serializada por `pg_advisory_xact_lock` por conta, sem estourar o teto com pedidos paralelos); conta soft-deletada não recebe. **Conta pré-registrada por terceiro:** o reenvio PÚBLICO (sem sessão) não manda link de confirmação e sim um link de **"conclua seu cadastro"** (token do reset de senha): concluí-lo define a senha de quem é dono do e-mail, revoga as sessões e marca a conta como confirmada — a senha de quem pré-registrou morre ali. O e-mail do cadastro/login continua sendo link de confirmação. Resíduo aceito: a vítima que clica num e-mail de confirmação que não pediu (disparado por cadastro/login de terceiro) confirma a conta com a senha dele; proposta de follow-up: purgar contas não confirmadas com mais de ~48h no cron de purge de contas. **Handle:** com a confirmação ligada a conta nasce com handle aleatório de espera (`pendente-<16>`) e só ganha o derivado do nome ao provar o e-mail (confirmação ou link de senha), com as mesmas regras de colisão; até lá fica fora de `/api/u/<handle>` (perfil, seguidores, seguindo) e da busca de Cozinheiros — senão o `-2` da desambiguação e o perfil denunciariam o e-mail. **Google:** `accountLinking.disableImplicitLinking: true` — sem isso quem controla um Google com o e-mail da vítima (ou pré-registra o e-mail via Google) se vincularia sozinho à conta de senha. Custo: login com Google num e-mail que já tem conta de senha responde "account not linked" (como já acontecia com contas não confirmadas); conta criada pelo Google continua entrando pelo Google. **CSRF de login aceito:** o link de confirmação loga quem o abre — repassado, loga o destinatário na conta de quem o enviou (mesmo custo do link mágico; o e-mail diz para ignorar se não pediu). Falha de envio do e-mail de conta (Brevo recusou/sem config) gera **uma** linha `warn` sem PII. A migração **0067** marca como confirmadas **todas as contas que existiam quando ela rodou** (nunca tiveram como confirmar; senão ficariam trancadas) (o vínculo implícito Google↔conta de mesmo e-mail que isso habilitaria está desligado — ver **Google** abaixo). **Gate (fail-safe):** a confirmação só é EXIGIDA quando o e-mail de conta pode sair — `getMailer().canSendAccountEmail()` (`BREVO_API_KEY` + remetente `AUTH_MAIL_FROM`/`DSAR_MAIL_FROM`), lido ao construir a instância do Better Auth. Sem isso o cadastro segue **como antes de #470** (loga direto, 200 com token; conta não confirmada entra), porque exigir confirmação sem canal de e-mail trancaria toda conta nova — e o `/sign-up/email` **continua enumerando** (422) até a env entrar; a correção liga sozinha no deploy seguinte. A migração 0067 roda de qualquer jeito. **Não é "ninguém trancado":** contas criadas **não confirmadas depois da 0067** — na janela entre a migração do build e o deploy ir ao ar, e durante todo o período com o gate desligado — precisam confirmar o e-mail no próximo login quando o gate ligar (o `sendOnSignIn` manda o link na hora, com a senha certa) e, até confirmar, ficam fora das superfícies públicas. Resíduo aceito: o caminho de conta nova faz alguns INSERTs a mais que o de conta existente (diferença de tempo pequena frente ao hash da senha, que os dois fazem; rate limit de 5/min por IP no cadastro).
