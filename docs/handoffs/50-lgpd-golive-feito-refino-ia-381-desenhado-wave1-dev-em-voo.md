...
3. 🟢 **Dev do #381 — Wave 1 TERMINOU LIMPO: PR #429 (#420 fundação) + PR #428 (#424 imagem), 0 achados de review adversarial.** **1ª coisa a fazer:** conferir CI e **mergear #429 (#420) primeiro** (é o seam), depois #428; daí lançar a Wave 2 (§3).
...
**Wave 1 TERMINOU LIMPO (review adversarial 3-lentes = 0 blocker/major nos dois):**
- **PR #429 — #420 fundação** (branch `feat/420-ia-seam-composicao`, **COM migração 0048**): seam `buildSystemPrompt(mode, axes)` PURO + registro extensível vazio (back-compat byte-a-byte) + os 3 prompts-base enriquecidos + `GenerationInput.axes?` opcional threaded nas 3 rotas + regenerate + **carimbo `generation.prompt_stamp` jsonb** ({version, axes}). `RecipeGenSchema`/classify intactos.
- **PR #428 — #424 imagem** (branch `feat/424-variedade-imagem`, sem migração): `STYLE_PREAMBLE` único → biblioteca composável + rotação determinística (FNV-1a puro do `recipeId` salgado por eixo) + mapa cozinha→convenção visual; prato-sujeito preservado; refino intacto. Minor sinalizado (não bloqueante): o mapa cozinha→visual é HARDCODED no código — o ADR-0029 rejeitou hardcode-por-cozinha pra a VOZ de TEXTO (dec.3, vai pro `vocabulary_term`), mas a dec.5 (imagem) não fixou storage e o mapa cobre 1:1 os 15 slugs ativos; aceitável no escopo, migrar pra data-driven é follow-up se quiser.

**Passo 1 — mergear (foreground):** confira CI (`gh pr checks 429` / `428`; watcher: trate `""`/null como PENDING). **Mergeie #429 (#420) PRIMEIRO** (é o seam), depois #428. Migração 0048 aplica on-deploy.

**Assinatura do seam pra Wave 2 (do `seamNotes` do #420 — use isto pra plugar cada eixo em 2 pontos de `src/domain/briefing.ts`, sem tocar mais nada):**
- (a) adicione UM campo OPCIONAL em `type PromptAxes` (ex.: `nivelChef?: NivelChef`) — manter opcional preserva o back-compat de `NEUTRAL_AXES`;
- (b) adicione UM item em `const AXIS_FRAGMENT_CONTRIBUTORS: readonly AxisFragmentContributor[]`, assinatura EXATA `AxisFragmentContributor = (axes: PromptAxes) => string | null` (retorna o fragmento quando o eixo está ativo, senão null; ordem do array = ordem de anexação ao base). `buildSystemPrompt` compõe automático.
- A borda (3 rotas + regenerate) hoje faz `const axes = NEUTRAL_AXES` — Wave 2 troca por ler o eixo do body/perfil ali.
- **BUMPAR `PROMPT_VERSION`** (hoje 1) quando o prompt-base OU o registro mudar de forma material — é ortogonal ao `SCHEMA_VERSION_RECEITA`. O carimbo `generation.prompt_stamp={version,axes}` correlaciona valores de eixo concretos com save/estrela.
- FakeClaudeClient ignora `axes` (como já ignora `cozinhaSlugs`).
...
Continuar o dev do refino da camada de IA do Refogando (#381). Leia docs/handoffs/50-lgpd-golive-feito-refino-ia-381-desenhado-wave1-dev-em-voo.md para o contexto completo. Estado: o desenho está fechado no ADR-0029 (mergeado) e nas issues #420–#426; a LGPD já foi ao ar (só faltam ações humanas do dono). A Wave 1 do dev já rodou e terminou LIMPA (review adversarial 0 achados): PR #429 (#420 fundação — seam buildSystemPrompt + prompt-base enriquecido + carimbo generation.prompt_stamp, COM migração 0048) e PR #428 (#424 variedade de imagem). PRIMEIRO PASSO: confira a CI dos dois (`gh pr checks 429` e `428`) e MERGEIE #429 (#420) PRIMEIRO — é o seam que todo o resto compõe — depois #428. Assim que #420 estiver na main, lance a Wave 2 em paralelo (#421 Nível de habilidade, #422 cozinha-como-voz, #423 Variação de geração/gerar-2, #425 comparador admin), branchando de main-com-#420; cada eixo pluga em src/domain/briefing.ts adicionando um campo opcional em PromptAxes + um item em AXIS_FRAGMENT_CONTRIBUTORS (assinatura (axes)=>string|null) e trocando NEUTRAL_AXES por ler o eixo na borda — ver a seção 'Assinatura do seam' do handoff. Gerencie no merge as colisões de src/domain/briefing.ts (aditivas, rebase do 2º) e de migração (rebase + db:generate dentro da worktree). Invariante duro: Opus 4.8 rejeita temperature/top_p/seed — variedade vem só do prompt; mantenha o RecipeGenSchema, os selos de IA e o prato-como-sujeito. O dono quer paralelizar o máximo e finalizar logo, sempre em português, mergeando no foreground quando a CI ficar verde. #426 (tradução real) é track própria (relaciona #187), não é parte do #381 — deixe por último. Padrão de dev: Workflow em worktrees (implement→review adversarial 3-lentes→fix→PR).
