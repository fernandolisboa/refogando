...
- **Controlador**: Fernando Lisboa, pessoa física responsável pelo Refogando (sem pessoa jurídica constituída; é como a Política publicada o identifica). **Encarregado (DPO)**: Fernando Lisboa, contato `privacidade@refogando.com`. **[VALIDAR COM O ADVOGADO]** se o controlador pessoa física se enquadra como agente de tratamento de pequeno porte (Res. CD/ANPD 2/2022).
...
- **Data**: 2026-07-01 (atualizado em 2026-09-24)
- **Versão**: 2 (rascunho pré-sign-off; §4 atualizado com as salvaguardas implementadas após 2026-07-01)
...
**Conclusão do balanceamento:** o interesse do controlador **prevalece** sobre os direitos e liberdades do titular, porque (a) o tratamento é **a favor da fonte** — no recorte pessoal, atribuição é um direito moral do próprio autor, não uma apropriação; (b) a **expectativa é alta** (nome de autor publicado na própria página) e o **impacto é leve** (dado comum, cópia privada, sem perfil/decisão); (c) a **minimização é efetiva** (só nome + URL; foto e headnote fora); e (d) há **salvaguardas concretas no código**, inclusive um caminho de **oposição** (remoção do nome, com a atribuição sobrevivendo rebaixada ao host). Some-se a isso que, na maioria das importações, `source_name` guarda uma **marca/veículo** e, portanto, **nem há dado pessoal de pessoa natural** em jogo. O balanceamento respeita as legítimas expectativas do titular e os direitos e liberdades fundamentais exigidos pelo Art. 10, II. **[VALIDAR COM O ADVOGADO]** a conclusão. (O gap de exercício de direitos apontado na versão 1, em que só o dono da importação conseguia remover o nome, foi fechado: o autor externo tem canal público e o operador consegue atendê-lo; ver §4.)

---

## 4. Salvaguardas adotadas (checklist)

- [x] **Transparência ativa (informar na política de privacidade)** — **Implementado.** A Política está publicada em `/privacidade` (texto em `src/i18n/messages/pt-BR.ts`, chave `privacidade`); o item 3.4 e a Parte (b) descrevem esta coleta, a finalidade e a base legal. Falta o sign-off jurídico do texto.
- [x] **Mecanismo de oposição fácil** — **Implementado para os dois titulares.** (a) O **autor externo** usa o formulário público em `/seus-direitos` (`POST /api/legal/takedown`, sem login, sem exigir documento) ou o e-mail `privacidade@refogando.com`; o operador executa a remoção com `POST /api/admin/attribution/clear` (remove o nome em todas as receitas importadas que casam por nome ou URL, inclusive privadas de qualquer usuário) e, se necessário, `POST /api/admin/attribution/escalate` (desvincula a URL ou apaga a receita importada). (b) O **dono da receita** continua podendo remover o nome sozinho (`POST /api/recipes/[id]/clear-attribution`). Prazo: 15 dias, com alertas automáticos aos 10/13/15 dias.
...
- [ ] **Limitação de retenção** — **GAP.** A Política publicada diz que a atribuição dura enquanto a receita importada existir ou até o pedido de remoção; não há prazo próprio. Continua aberto (issue #473) quanto aos **tickets de takedown**, que guardam a mensagem e o e-mail do solicitante sem prazo de expurgo. **[VALIDAR COM O ADVOGADO]**
...
- [x] **Registro (accountability)** — cada remoção feita pelo operador grava `DSAR_FULFILLED` na tabela `dsar_audit_event`, só inserção, com **hash** SHA-256 do que foi removido (nunca o nome em claro).
...
- [x] **Outras — camada expressiva excluída**: foto (nasce sem imagem) e `description`/headnote (nasce em branco) nunca são copiados.
- [x] **Outras — coleta educada e por ação explícita**: `robots.txt` (RFC 9309), `User-Agent` identificado, rate-limit ~1 req/s/domínio, fetch só sob clique do usuário, allowlist curada e denylist de domínios vetados por ToS (`TOS_DENYLIST`).
- [x] **Outras — atribuição a favor do titular**: na remoção padrão, `source_url` fica e o crédito "ver no site" persiste rebaixado ao host. A remoção integral (URL ou receita) existe, mas só por decisão do operador (ver §6).

**GAPs remanescentes:** (1) prazo de retenção dos tickets de takedown e do log de auditoria (#473); (2) sign-off jurídico do texto publicado. Os GAPs de 2026-07-01 sobre política, canal público e encarregado foram fechados. A feature nasce **desligada** e só liga com três controles: configuração admin `web_search_enabled` (falsa por padrão), allowlist não vazia e `WEB_SEARCH_API_KEY`.
...
**Responsável pela decisão (Encarregado / Jurídico):** ____________________________ (Encarregado nomeado: Fernando Lisboa; sign-off jurídico pendente)
