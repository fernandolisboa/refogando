...
export async function operatorClearSourceAttribution(input: {
  ...
  if (name === '' && url === '') return { applied: false, matched: 0, recipeIds: [] }
  ...
  // Prévia: só lê e conta — sem transação, sem mutar, sem evento de auditoria.
  if (!apply) {
    const rows = await db.select(columns).from(recipe).where(where)
    const removable = rows.filter(hasRemovableSourceName)
    return { applied: false, matched: rows.length, recipeIds: removable.map((r) => r.id) }
  }
  ...
  return db.transaction(async (tx) => {
    ...
    const removable = rows.filter(hasRemovableSourceName)
    if (removable.length === 0) return { applied: true, matched: rows.length, recipeIds: [] }
    ...
    // `removedSourceName` = nomes DISTINTOS de fato removidos (ordenados p/ hash estável). No caso comum
    // (match por um nome, ou por URL = uma fonte) é um só; o campo NUNCA é persistido em claro — só entra
    // no HASH do payload (senão a auditoria copiaria o dado que se pediu para apagar — #395 §5).
    const removedSourceName = Array.from(new Set(removable.map((r) => r.sourceName as string)))
      .sort()
      .join(' | ')
    ...
    return { applied: true, matched: rows.length, recipeIds: ids }
  })
}
