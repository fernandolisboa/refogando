 ...
 * itálico, sem borda); `aviso` = âmbar de restrição (NUNCA vermelho). `bg-accent`/`bg-muted`
 * nus jamais — a erva é só do selo do Catálogo e `muted` é texto, não superfície.
 */
  ...
  'inline-flex items-center gap-1 rounded-sm border px-2 py-0.5 text-xs font-medium transition-colors duration-150 ease-out w-fit whitespace-nowrap shrink-0 [&>svg]:size-3',
  {
    variants: {
      variant: {
        ...
        aviso: 'border-aviso-fg/30 bg-aviso-bg text-aviso-fg',
      },
    },
    defaultVariants: {
      variant: 'default',
