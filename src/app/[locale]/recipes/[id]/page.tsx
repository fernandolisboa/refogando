 ...
 * locale-no-caminho (ADR-0020/#228) separa a CHROME (idioma exibido = `params.locale` do path)
 * do CONTEÚDO (qual tradução BUSCAR = chrome, OU o `?original=<locale>` da jornada "ver o
 * original"). Monta a URL absoluta, faz `fetch` da própria rota `GET /api/recipes/[id]` (ADR-0010:
 * a UI consome ROUTE HANDLERS, não Server Actions; não reimplementa domínio) ENCAMINHANDO o
 * cookie de sessão (senão a rota vê requisição anônima e o dono nunca lê a própria receita
 * privada) e renderiza o shape `RecipeView` que a rota devolve.
 *
 ...
 * global (Suspense). A lógica testável (status→efeito, chrome vs conteúdo) vive em helpers PUROS
 * (`handleResponse`, `resolvePageLocale`, `resolveContentLocale`) — `headers()` lança fora do
 * request scope no jsdom, então a page não é testável lá, mas seus branches são.
 *
...
export default async function RecipeDetailPage({
  ...
  // Locale-no-caminho (ADR-0020): o idioma da CHROME (mensagens + `<html lang>`) vem do
  // SEGMENTO DA URL (`params.locale`), que o proxy garante prefixado e canônico. cookie/
  // Accept-Language ficam só como rede de segurança (defesa em profundidade — nunca tela
  // quebrada se o path chegar inválido).
  ...
  // Conteúdo (qual tradução BUSCAR) é distinto da chrome: a jornada "ver o original" (#23)
  // linka `?original=<locale>` pra LER o corpo no idioma-fonte SEM trocar a chrome/o path.
  // Sem `?original`, o conteúdo segue a chrome. (O `?locale` legado morreu com o path-wins;
  // o escape explícito `?original` o substitui até o slug-por-locale #230.)
  const contentLocale = resolveContentLocale({ pageLocale: locale, original: sp.original })

  const base = await getBaseUrl()
  const url = `${base}/api/recipes/${encodeURIComponent(id)}?locale=${encodeURIComponent(contentLocale)}`
